...
//  @deprecated("The 'govuk-tax' is an unnecessary level of configuration please use ServicesConfig.services", "24.11.14")
  ...
  protected lazy val defaultProtocol: String =
    runModeConfiguration
      ...
      .getOrElse(
        runModeConfiguration
          .getString(s"$services.protocol")
          .getOrElse("http"))

  protected def config(serviceName: String) =
    runModeConfiguration
      .getConfig(s"$rootServices.$serviceName")
      .getOrElse(
        runModeConfiguration
          .getConfig(s"$services.$serviceName")
          .getOrElse(runModeConfiguration
            .getConfig(s"$playServices.$serviceName")
  ...
  def getConfInt(confKey: String, defInt: => Int) =
    runModeConfiguration
      ...
      .getOrElse(
        runModeConfiguration
          .getInt(s"$services.$confKey")
          .getOrElse(runModeConfiguration.getInt(s"$playServices.$confKey").getOrElse(defInt)))

  def getConfBool(confKey: String, defBool: => Boolean) =
    runModeConfiguration
      .getBoolean(s"$rootServices.$confKey")
      .getOrElse(
        runModeConfiguration
          .getBoolean(s"$services.$confKey")
          .getOrElse(runModeConfiguration.getBoolean(s"$playServices.$confKey").getOrElse(defBool)))
  ...
  def getDuration(key: String) =
    runModeConfiguration.getString(key).map(Duration.create).getOrElse(configNotFoundError(key))
